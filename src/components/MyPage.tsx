...
import { Settings, Heart, Upload, User, MapPin, Star } from "lucide-react";
...
interface MyPageProps {
  stores: Shop[];
  onNavigate: (page: "detail", store: Shop) => void;
...
export function MyPage({ stores, onNavigate, onLike }: MyPageProps) {
  ...
  const currentUser = {
    name: "八太茶",
    username: "hatacha",
    avatar_url: "https://i.pravatar.cc/128?u=current_user",
  ...
  const myPosts = stores.filter(
    (store) =>
      store.user.username === "ramen_taro" ||
      store.user.username === "cafe_sato"
  );

  const StoreGrid = ({ storeList }: { storeList: Shop[] }) => (
