...
const ShopList: React.FC<ShopListProps> = ({ shops, onNavigate, onLikeToggle, isLiking, user, onEdit }) => {
  return (
    <div className="container mx-auto px-4 py-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 justify-items-center">
        {shops.map((shop) => (
          <ShopCard key={shop.id} shop={shop} onNavigate={onNavigate} onLikeToggle={onLikeToggle} isLiking={isLiking} user={user} onEdit={onEdit} />
        ))}
      </div>
    </div>
  );
