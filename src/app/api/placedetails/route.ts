...
import { Json, Database } from "@/lib/database.types"; // Import Json type
...
interface ShopWithTypes extends ShopTableRow {
  types: string[] | null;
}
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const { data, error: cacheError } = await supabase
      ...
      .eq("place_id", placeId)
      .single();
    const cachedShop = data as ShopWithTypes | null;
    ...
    if (cachedShop && !cacheError) {
      if (cachedShop.api_last_updated) {
        ...
        const now = new Date();
        const hoursDiff =
          Math.abs(now.getTime() - lastUpdated.getTime()) / (1000 * 60 * 60);
        ...
        if (hoursDiff < CACHE_DURATION_HOURS) {
          ...
          const responseData: ShopData = {
            ...
            business_hours_weekly: cachedShop.business_hours_weekly as
              | BusinessHours[]
              | null,
            ...
            types: cachedShop.types as string[] | null,
          };
    ...
    console.log(
      `Fetching fresh data from Google Places API (New) for place_id: ${placeId}`
    );
    ...
    const googlePlacesUrl = `https://places.googleapis.com/v1/places/${placeId}?languageCode=ja`;
    const fieldMask =
      "displayName,regularOpeningHours,rating,photos,internationalPhoneNumber,priceLevel,types";
    ...
    if (googleData.error) {
      ...
      return NextResponse.json(
        ...
        { status: googleData.error.code || 500 }
      );
    }
    ...
    const businessHoursWeekly: BusinessHours[] =
      result.regularOpeningHours?.weekdayDescriptions?.map((text: string) => {
    ...
    const priceRange = result.priceLevel
      ? priceLevelMap[result.priceLevel]
      : null;

    const shopName =
      typeof result.displayName === "object" && result.displayName !== null
    ...
    const newShopData: ShopData = {
      ...
      business_hours_weekly:
        businessHoursWeekly.length > 0 ? businessHoursWeekly : null,
    ...
    const { error: upsertError } = await supabase.from("shops").upsert(
      {
        ...
        business_hours_weekly:
          newShopData.business_hours_weekly as unknown as Json,
        ...
        types: newShopData.types as unknown as Json,
      },
      { onConflict: "place_id" }
    );
